        ...
        if "@" in link_without_prefix:
            try:
                ...
                if ":" in server_part:
                    ...
                    raise ValueError("Invalid server part: missing port")
            except Exception as e:
                raise ValueError(f"Failed to parse Trojan parts: {e}")
        else:
            raise ValueError("Invalid Trojan link format: missing @")
    except Exception as e:
        raise ValueError(f"Error parsing Trojan link: {e}")
...
def _convert_trojan_link(trojan_link):
    try:
...
def find_trojan_link(text, regex):
    ...
    for link in links:
        ...
        result = _convert_trojan_link(link)
        if result:
            print("result: ", result)
...
if __name__ == "__main__":
    ...
    result = _convert_trojan_link(trojan_link)
    print(result)
